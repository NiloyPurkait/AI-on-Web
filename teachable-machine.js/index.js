...
function updateTable(objD) {
  for (var key in objD) {
      ...
      if (objD.hasOwnProperty(key)) {
          var tr = "<tr>";
          if (objD[key].toString().substring(objD[key].toString().indexOf('.'), objD[key].toString().length) < 2) objD[key] += "0";

          tr += "<td>" + key + "</td>" + "<td>" + objD[key].toString() + "</td></tr>";
          tbody.innerHTML += tr;
...
async function appSimpleRT() {
  ...
  async function app() {
    ...
    document.getElementById('class-a').addEventListener('click', () => addExampleImage(0));
    document.getElementById('class-b').addEventListener('click', () => addExampleImage(1));
    document.getElementById('class-c').addEventListener('click', () => addExampleImage(2));
    document.getElementById('reset_classes').addEventListener('click', () =>  resetModel());
    ...
    while (true) {
      if (classifier.getNumClasses() > 0) {
        ...
        const confidence = Math.round(result.confidences[result.label] * 100)/100;
